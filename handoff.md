...
- ⚠️ `v1` → `v2`: на одному пристрої прапор `v1` встановився, але XP лишився старий —
  ймовірно через «Відновити» зі старого бекапу (import перезаписує `oxford_xp_v1`, але
  не новий прапор). Тому прапор перейменовано на `v2`, щоб міграція пройшла заново.
  Порада користувачу: після фіксу зробити свіжий бекап, щоб не відкотити старим.
