...
**Перерахунок XP з формули (сесія 8):** прапор `oxford_xp_formula_v1`. Одноразово
...
перенарахування (баги вище) і ДОБИЛО XP за ранні дні, коли трекінгу XP ще не було
(back-fill). Замінив старий ретро-блок 27–30 травня (`oxford_xp_day_retro_v1`). Не чіпати.
Далі total і per-day тримаються в синхроні через `addXp` (saveXp + trackDayXp).
